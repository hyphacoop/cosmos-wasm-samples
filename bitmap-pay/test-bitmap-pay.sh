...
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":200,"y":0,"z":"0011AA"}}'  --home $CHAIN_HOME --from $WALLET --amount ${cost}uatom --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID

echo "> Executing the contract (unhappy path: insufficient funds)"
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":0,"y":0,"z":"0011AA"}}'  --home $CHAIN_HOME --from $WALLET --amount 10uatom --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID

echo "> Setting (0,0) for the first time"
cost=$($CHAIN_BINARY q wasm contract-state smart $contract '{"get_cost":{"x":0,"y":0}}' -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.data.cost')
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":0,"y":0,"z":"0011AA"}}' --home $CHAIN_HOME --from $WALLET --amount ${cost}uatom --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
$CHAIN_BINARY q bank balances $RECIPIENT -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.balances'
...
cost=$($CHAIN_BINARY q wasm contract-state smart $contract '{"get_cost":{"x":0,"y":0}}' -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.data.cost')
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":0,"y":0,"z":"0033CC"}}' --home $CHAIN_HOME --from $WALLET --amount ${cost}uatom --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
$CHAIN_BINARY q bank balances $RECIPIENT -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.balances'
...
cost=$($CHAIN_BINARY q wasm contract-state smart $contract '{"get_cost":{"x":0,"y":1}}' -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.data.cost')
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":0,"y":1,"z":"EE1122"}}' --home $CHAIN_HOME --from $WALLET --amount ${cost}uatom --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
$CHAIN_BINARY q bank balances $RECIPIENT -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.balances'
