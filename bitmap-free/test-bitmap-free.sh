...
tx_hash=$($CHAIN_BINARY tx wasm store $FILENAME --home $CHAIN_HOME --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.txhash')
...
tx_hash=$($CHAIN_BINARY tx wasm instantiate $code_id '{"x_size":16,"y_size":16}' --home $CHAIN_HOME --label "bitmap" --no-admin --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID | jq -r '.txhash')
sleep $COMMIT_TIMEOUT
...
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":200,"y":1,"z":"0011AA"}}'  --home $CHAIN_HOME --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
set -e
echo "> Executing the contract (set 0,1)"
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":0,"y":0,"z":"0011AA"}}' --home $CHAIN_HOME --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
echo "> Executing the contract (set 1,0)"
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":1,"y":0,"z":"0022BB"}}' --home $CHAIN_HOME --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
echo "> Executing the contract (set 1,1)"
$CHAIN_BINARY tx wasm execute $contract '{"set":{"x":1,"y":1,"z":"0033CC"}}' --home $CHAIN_HOME --from $WALLET --gas auto --gas-adjustment 3 --gas-prices $GAS_PRICE -y -o json --node $NODE --chain-id $CHAIN_ID
sleep $COMMIT_TIMEOUT
